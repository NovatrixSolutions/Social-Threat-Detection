...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Simple report generation button (your inline PDF logic in HTML still works)
...
/* ---------- CONFIDENCE CHART HOOK ---------- */

function sendToChart(detections, meta) {
    const frame = document.getElementById("threatChartFrame");
    if (!frame || !frame.contentWindow) {
        console.error("Chart iframe not found or not ready.");
        return;
    }

    const payload = {
        type: "THREAT_CONFIDENCE_DATA",
    ...
    // Safety: resend after a short delay in case iframe JS wasn't ready yet
    setTimeout(() => {
        try {
            frame.contentWindow.postMessage(payload, "*");
        } catch (e) {
            console.warn("Retry sendToChart failed:", e);
        }
    }, 300);
}

/* ---------- SHOW REAL API RESULTS (WITH CHART) ---------- */
...
function showRealResults(results, keywords, platform) {
    ...
    if (results.success && detections.length > 0) {
        ...
        // Send data to confidence chart
        sendToChart(detections, {
            platform: platform.toUpperCase(),
            query: keywords,
            scannedCount: scannedCount,
            threatsCount: threatsFound
        });
...
//    }
//
//    // If iframe not loaded yet, wait for load event then send
...
//        });
//
//        // ✅ send data to the confidence chart
//        sendToChart(detections, {
//            platform: platform.toUpperCase(),
//            query: keywords,
//            scannedCount: scannedCount,
...
//            </div>
//        `;
//        // on error we just don't touch the chart
//    }
//
//    resultsSection.scrollIntoView({ behavior: 'smooth' });
//}
